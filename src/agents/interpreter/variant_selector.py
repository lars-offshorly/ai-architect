...
class _VariantSelectionContext:
...
def select(
    ...
    context = _build_context(extracted.classification_signals, user_message)

    scored: list[tuple[BundleVariantDefinition, int]] = []
    for variant in variants:
        score = _score_variant(variant=variant, context=context)
...
def _score_variant(
    *,
    ...
    context: _VariantSelectionContext,
) -> int:
    keyword_hits = _overlap_count(variant.keywords, context.extracted_keywords)
    phrase_hits = _phrase_match_count(variant.keywords, context.user_message_lower)
    entity_hits = _overlap_count(variant.typical_entities, context.extracted_entities)
    intent_hits = _overlap_count(variant.typical_intents, context.extracted_intents)
    industry_hits = _overlap_count(variant.industry_hints, context.extracted_domains)
    anti_hits = _phrase_match_count(variant.anti_keywords, context.signal_pool)
...
def _build_context(
    ...
    user_message: str,
) -> _VariantSelectionContext:
    return _VariantSelectionContext(
...
def _concatenate_signals(user_message: str, cs: ClassificationSignals) -> str:
