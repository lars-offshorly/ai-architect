...
class TestPreselectedBundleBypassesClassification:
    ...
    async def test_mock_suggestion_has_full_confidence(self) -> None:
        ...
        suggested: SuggestedBundles = result["suggested"]  # type: ignore[assignment]
        assert suggested.top() is not None
        assert suggested.top().bundle_key == _PRESELECTED_KEY
        assert suggested.top().confidence == 1.0
        assert suggested.top().reasoning == "Pre-selected by user"
...
class TestForcePreviewing:
    ...
    @pytest.mark.asyncio
    async def test_force_preview_falls_back_to_all_microservices_when_no_bundle(
        self,
    ) -> None:
        ...
        assert result["status"] == "ready_for_preview"
        assert result.get("bundle_key") == "all_microservices"
        assert result.get("preview_type") == "early"
        assert result.get("warning") is not None
